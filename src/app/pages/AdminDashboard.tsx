  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 lg:px-8 py-8">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          <TabsList className="mb-8 bg-secondary flex flex-wrap gap-1">
            ...
            <TabsTrigger value="users" className="text-red-400 data-[state=active]:bg-primary data-[state=active]:text-white">
              Пользователи
              <span className="ml-2 bg-primary text-white rounded-full px-2 py-0.5 text-xs">{users.length}</span>
            </TabsTrigger>
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <div className="bg-gradient-to-br from-blue-500/10 to-blue-500/5 rounded-2xl p-6 border-2 border-blue-500/20">
                <div className="flex items-center gap-3 mb-4">
                  <div className="bg-blue-500/20 p-2 rounded-lg"><Users className="w-5 h-5 text-blue-500" /></div>
                  <h3 className="font-semibold text-red-400">Всего клиентов</h3>
                </div>
              ...
              <div className="bg-gradient-to-br from-green-500/10 to-green-500/5 rounded-2xl p-6 border-2 border-green-500/20">
                <div className="flex items-center gap-3 mb-4">
                  <div className="bg-green-500/20 p-2 rounded-lg"><UserCheck className="w-5 h-5 text-green-500" /></div>
                  <h3 className="font-semibold text-red-400">Активные абонементы</h3>
                </div>
              ...
              <div className="bg-gradient-to-br from-purple-500/10 to-purple-500/5 rounded-2xl p-6 border-2 border-purple-500/20">
                <div className="flex items-center gap-3 mb-4">
                  <div className="bg-purple-500/20 p-2 rounded-lg"><DollarSign className="w-5 h-5 text-purple-500" /></div>
                  <h3 className="font-semibold text-red-400">Общий доход</h3>
                </div>
              ...
              <div className="bg-gradient-to-br from-orange-500/10 to-orange-500/5 rounded-2xl p-6 border-2 border-orange-500/20">
                <div className="flex items-center gap-3 mb-4">
                  <div className="bg-orange-500/20 p-2 rounded-lg"><Activity className="w-5 h-5 text-orange-500" /></div>
                  <h3 className="font-semibold text-red-400">Посещений сегодня</h3>
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="bg-card rounded-2xl p-6 border border-border">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <Button variant="ghost" size="sm" onClick={() => setActiveTab("trainers")} className="text-primary">Все тренеры →</Button>
                </div>
                <div className="space-y-3">
            ...
            <div className="bg-card rounded-2xl p-6 border border-border">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-semibold text-red-400">Недавние записи</h3>
                <Button variant="ghost" size="sm" onClick={() => setActiveTab("trainings")} className="text-primary">Все записи →</Button>
              </div>
              {bookings.length > 0 ? (
                <div className="space-y-3">
            ...
            <div className="bg-card rounded-2xl p-6 border border-border">
              <div className="flex items-center justify-between mb-6 flex-wrap gap-4">
                ...
                <div className="relative">
                  ...
                  <Input placeholder="Поиск..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-10 w-80 bg-input-background text-red-400 border-border" />
                </div>
              </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="text-left py-3 px-4 text-sm font-semibold text-red-400">Имя</th>
                      <th className="text-left py-3 px-4 text-sm font-semibold text-red-400">Email</th>
                      <th className="text-left py-3 px-4 text-sm font-semibold text-red-400">Телефон</th>
                      <th className="text-left py-3 px-4 text-sm font-semibold text-red-400">Роль</th>
                      <th className="text-left py-3 px-4 text-sm font-semibold text-red-400">Статус</th>
                      <th className="text-left py-3 px-4 text-sm font-semibold text-red-400">Регистрация</th>
                      <th className="text-right py-3 px-4 text-sm font-semibold text-red-400">Действия</th>
                    </tr>
                  </thead>
                  <tbody>
                        ...
                        <td className="py-3 px-4">
                          <div className="flex items-center justify-end gap-2">
                            {userItem.status === 'active' ? (
                              <Button size="sm" variant="outline" onClick={() => handleUserAction(userItem.id, 'block')} className="text-red-400" title="Заблокировать"><Ban className="w-4 h-4" /></Button>
                            ) : (
                              <Button size="sm" variant="outline" onClick={() => handleUserAction(userItem.id, 'unblock')} className="text-red-400" title="Разблокировать"><CheckCircle className="w-4 h-4" /></Button>
                            )}
                            <Button size="sm" variant="outline" onClick={() => handleUserAction(userItem.id, 'delete')} className="text-red-400" title="Удалить"><Trash2 className="w-4 h-4 text-red-500" /></Button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          <TabsContent value="trainers" className="space-y-6">
            <div className="bg-card rounded-2xl p-6 border border-border">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-semibold text-red-400">Тренерский состав</h3>
                <Button className="bg-primary hover:bg-primary/90 text-white"><Plus className="w-4 h-4 mr-2" />Добавить тренера</Button>
              </div>
                ...
                {trainers.map((trainer) => (
                  <div key={trainer.id} className="border border-border rounded-xl p-5 space-y-4 bg-card">
                    ...
                    <div className="flex gap-2 pt-2">
                      <Button size="sm" variant="outline" className="flex-1 text-red-400"><Edit className="w-4 h-4 mr-2" />Редактировать</Button>
                      <Button size="sm" variant="outline" className="text-red-500"><Trash2 className="w-4 h-4" /></Button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <TabsContent value="trainings" className="space-y-6">
            <div className="bg-card rounded-2xl p-6 border border-border">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-semibold text-red-400">Все тренировки</h3>
                <Button className="bg-primary hover:bg-primary/90 text-white"><Plus className="w-4 h-4 mr-2" />Добавить тренировку</Button>
              </div>
          ...
          <TabsContent value="memberships" className="space-y-6">
            <div className="bg-card rounded-2xl p-6 border border-border">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-semibold text-red-400">Активные абонементы</h3>
                <Button className="bg-primary hover:bg-primary/90 text-white"><Plus className="w-4 h-4 mr-2" />Создать абонемент</Button>
              </div>
              <div className="space-y-4">
                    ...
                    <div key={membership.id} className="flex flex-col md:flex-row md:items-center justify-between p-5 bg-secondary rounded-xl gap-4">
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap items-center gap-4 text-sm text-red-400">
                          ...
                          <div>Действителен с: <span className="font-semibold text-red-400">{format(new Date(membership.validFrom), "d MMMM yyyy", { locale: ru })}</span></div>
                          ...
                          <div>Посещений: <span className="font-semibold text-red-400">{membership.visitsLeft === 'unlimited' ? 'Безлимит' : membership.visitsLeft}</span></div>
                        </div>
                      </div>
          ...
          <TabsContent value="promotions" className="space-y-6">
            <div className="bg-card rounded-2xl p-6 border border-border">
              <div className="flex items-center justify-between mb-6">
                <h3 className="font-semibold text-red-400">Акции и предложения</h3>
                <Button onClick={handleAddPromotion} className="bg-primary hover:bg-primary/90 text-white"><Plus className="w-4 h-4 mr-2" />Добавить акцию</Button>
              </div>
              <div className="grid md:grid-cols-2 gap-6">
                {promotions.map((promo) => (
                  <div key={promo.id} className="border border-border rounded-xl p-5 space-y-4 bg-card">
                    <div className="flex items-start justify-between">
                      <div className="flex-1"><div className="font-semibold text-lg text-red-400 mb-1">{promo.title}</div><div className="text-sm text-red-400">{promo.description}</div></div>
      ...
      <ConfirmDialog
        ...
        description={actionType === 'delete' ? 'Пользователь и все его данные будут удалены. Это действие нельзя отменить.' : actionType === 'block' ? 'Пользователь не сможет войти в систему.' : 'Пользователь снова сможет войти в систему.'}
        confirmLabel={actionType === 'delete' ? 'Удалить' : actionType === 'block' ? 'Заблокировать' : 'Разблокировать'}
        cancelLabel="Отмена"
          ...
          <div className="bg-card rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto border border-border">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-semibold text-red-400 mb-2">Дата начала</label><input type="date" value={promoForm.validFrom} onChange={(e) => setPromoForm({ ...promoForm, validFrom: e.target.value })} className="w-full px-4 py-2 bg-input-background border border-border rounded-lg text-red-400" /></div>
                <div><label className="block text-sm font-semibold text-red-400 mb-2">Дата окончания</label><input type="date" value={promoForm.validUntil} onChange={(e) => setPromoForm({ ...promoForm, validUntil: e.target.value })} className="w-full px-4 py-2 bg-input-background border border-border rounded-lg text-red-400" /></div>
              </div>
              <div className="flex gap-3 pt-4">
                <Button onClick={handleSavePromotion} className="flex-1 bg-primary text-white">{editingPromo ? 'Сохранить' : 'Создать'}</Button>
                <Button variant="outline" onClick={() => setPromoDialogOpen(false)} className="flex-1 text-red-400">Отмена</Button>
              </div>
            </div>
          </div>
        </div>
